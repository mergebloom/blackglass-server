...
describe("Bun oracle and Rust protocol parity", () => {
  beforeAll(async () => {
    ...
    const initialized = Bun.spawnSync(
      [rustBinary, "user", "create", join(directory, "rust.sqlite"), "owner@example.test", "Parity owner"],
    ...
    const rustChild = Bun.spawn([rustBinary, "serve"], {
      cwd: root, stdout: "pipe", stderr: "pipe",
      env: { ...process.env, SELFHOST_BIND_HOST: "127.0.0.1", SELFHOST_CONTROL_PORT: String(ports[2]), SELFHOST_DATA_PORT: String(ports[3]), SELFHOST_DATA_HOST: `127.0.0.1:${ports[3]}`, SELFHOST_DATABASE: join(directory, "rust.sqlite"), SELFHOST_STAGING_DIR: join(directory, "uploads"), SELFHOST_PER_FILE_MAX: String(8 * 1024 * 1024), SELFHOST_ALLOWED_ORIGIN: "app://obsidian.md", SELFHOST_LOG_FORMAT: "pretty" },
    });
